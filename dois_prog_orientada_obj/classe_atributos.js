//Ex. de caso não entre um vetor como parametro a função ela retorne erro  
class quadrado {
    //Contructor de quadrados
    constructor(base, altura){
      //Metodo isNaN: não é numerico *** ESSE METODO CHECA SE O STRING CORRESPONDE A UM NUMERO E RETORNA O VALOR false = isNaN("37") / Q BOSTA DE FUNÇÃO!!
      //Bom para esse exercicio está bom... mas realmente decepcionante isso do isNaN....
      //Link da explicação (veja tabela de exemplos): https://developer.mozilla.org/pt-BR/docs/Web/JavaScript/Reference/Global_Objects/isNaN
      if (isNaN(base)) {
        //Aqui um sistema de correção de entradas... cosa nostra manja...
        base = this.setEntranceCorrectFormat(base);
        this.base = base;
      } else {
        this.base = base;
      }
      ...
      if (isNaN(altura)) {
        //Aqui um sistema de correção de entradas... cosa nostra manja...
        altura = this.setEntranceCorrectFormat(altura);
        this.altura = altura;
      } else {
        this.altura = altura;
      }
      //else if (typeof altura == Number){this.altura = altura}

      //Parametro opcional, por isso undefined, pode ser dado mas não há nada que obrigue a existir.
      this.cor = undefined;
      //Chama a função criadora de parametro calculado. Não sei se atualiza quando mudam os parametros base e altura...
      this.area = this.calculaArea(base, altura);
    }
    //Função legal que define como o proprio objeto será mostrado numa pagina.
    printHTML(){
        return `<div style='width:${this.base}px;height:${this.altura}px;background-color:${this.cor || "blue"}'></div>`;
    }
    //Função simples que calcula a are do quadrado dado na instância
    calculaArea(base, altura){
        const  Area = base*altura
        return Area
    }
    setEntranceCorrectFormat (x) {
...
//Por isso a linha com isNaN dentro da classe. A aula da LetsCode não avisou q numero que vem como string ex. "37" o isNaN não pega, vide constructo para + info.
//Aqui o bixo com as entradas corretas.
var Sqr = new quadrado("duzentos e vinte", 12);
console.log(Sqr)